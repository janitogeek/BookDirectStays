...
const AIRTABLE_TABLE_NAME = 'Directory Submissions'; // CORRECT: API key has access to this table
...
export const airtableService = {
  ...
  async getApprovedSubmissions(): Promise<Submission[]> {
    ...
    do {
      ...
      if (!response.ok) {
        console.error(`❌ Airtable API error on batch ${requestCount}:`, response.status, response.statusText);
        throw new Error(`Airtable API error: ${response.statusText}`);
      }

      const batchData = await response.json();
      const batchRecords: AirtableSubmission[] = batchData.records || [];
