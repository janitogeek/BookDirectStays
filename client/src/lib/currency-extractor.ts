...
export interface CountryCurrencyMapping {
  [countryName: string]: string; // country -> currency code
}

/**
 * Currency mapping with symbols and names
 */
 ...
 */
const COUNTRY_CURRENCY_MAP: CountryCurrencyMapping = {
  'United States': 'USD',
  'Canada': 'CAD',
  'Mexico': 'MXN',
  'United Kingdom': 'GBP',
...
/**
 * Extract all unique currencies from published submissions
 */
export function extractCurrenciesFromSubmissions(submissions: any[]): CurrencyInfo[] {
  const currencySet = new Set<string>();
  const currencyCountries: Record<string, Set<string>> = {};
  
  submissions.forEach(submission => {
    // Only process published submissions
    const isPublished = submission.status === 'published' || 
                       submission.status === 'Approved – Published' ||
                       submission.status === 'Approved - Published';
    
    if (!isPublished || !submission.currency) {
      return;
    }
    ...
    if (currency && currency.length > 0) {
      ...
      // Track which countries use this currency
      if (submission.geonamesRecord && typeof submission.geonamesRecord === 'string') {
        // Extract countries from Geonames record
        const countries = extractCountriesFromGeonamesRecord(submission.geonamesRecord);
        countries.forEach(country => {
          if (!currencyCountries[currency]) {
            currencyCountries[currency] = new Set();
          }
          currencyCountries[currency].add(country);
        });
      }
    }
  });
  
  // Convert to CurrencyInfo array
  return Array.from(currencySet).map(currency => {
    ...
    return {
      ...
      countries: Array.from(currencyCountries[currency] || [])
    };
 ...
 * Extract countries from Geonames record
 */
function extractCountriesFromGeonamesRecord(geonamesRecord: string): string[] {
  if (!geonamesRecord || typeof geonamesRecord !== 'string') {
    return [];
  }
  
  const records = geonamesRecord.split(';').map(record => record.trim()).filter(record => record.length > 0);
  const countries = new Set<string>();
  
  records.forEach(record => {
    const parts = record.split(',').map(part => part.trim());
    if (parts.length >= 2) {
      const country = parts[parts.length - 1]; // Last part is always country
 ...
 */
export function getAllCurrencies(submissions: any[]): CurrencyInfo[] {
  return extractCurrenciesFromSubmissions(submissions);
}

/**
 * Get country-currency mapping
 */
export function getCountryCurrencyMapping(): CountryCurrencyMapping {
